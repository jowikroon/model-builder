; Complete Samantha AI Windows Installer (No System Requirements)
; Simplified installer without restrictive system checks
...
; Finish page
!define MUI_FINISHPAGE_RUN "$INSTDIR\install-no-docker.bat"
...
Section "Samantha AI Core" SecCore
  ...
  DetailPrint "Installing Samantha AI files..."
  ...
  DetailPrint "Samantha AI core installation completed!"
SectionEnd
...
Section "Desktop Shortcuts" SecShortcuts
  ...
  ; Desktop shortcut
  CreateShortCut "$DESKTOP\Samantha AI.lnk" "$INSTDIR\install-no-docker.bat" "" "$INSTDIR\samantha.ico"
  
  ; Start menu shortcuts
  CreateDirectory "$SMPROGRAMS\Samantha AI"
  CreateShortCut "$SMPROGRAMS\Samantha AI\Install Samantha AI.lnk" "$INSTDIR\install-no-docker.bat" "" "$INSTDIR\samantha.ico"
...
Section "Quick Start Guide" SecGuide
  DetailPrint "Creating quick start guide..."
  ...
  FileWrite $0 "   - Installation may take 15-30 minutes$\r$\n$\r$\n"
  ...
  FileWrite $0 "   - If issues occur, restart your computer$\r$\n"
  FileWrite $0 "   - Ensure Docker Desktop is running$\r$\n"
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\install-no-docker.bat"
